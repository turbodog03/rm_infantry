    ...
    switch (chassis.mode)
    {
      ...
      case CHASSIS_FOLLOW_GIMBAL:
      {
         ...
         || ((gim.ctrl_mode == GIMBAL_NO_ACTION) && (gim.no_action_flag == 1)))
		{
			...
			//sprintf(mmp_buf,"%f %f",chassis.vw,yaw_relative_angle);
...
void chassis_pid_param_init(void)
{
  ...
  for (int k = 0; k < 4; k++)
  {
	pid_init(&pid_wheel_spd[k], 7000, 3000, 3.0f, 0.1, 0.0);
  }
  //底盘跟随PID参数设置
  pid_init(&pid_chassis_angle, MAX_CHASSIS_VR_SPEED, 40, 15.0f, 0.5f, 100.0f);
