...
pid_t pid_pit           = { 0 };
pid_t pid_yaw           = { 0 };
pid_t pid_pit_auto      = { 0 };
pid_t pid_yaw_auto      = { 0 };
...
//摩擦轮电机 PID 结构体定义
pid_t pid_shoot_left = { 0 };
pid_t pid_shoot_right = { 0 };
              ...
              float kp, float ki, float kd)
{
  pid->integral_limit = intergral_limit;
  pid->max_output     = max_out;
	pid->filtering_rate = flitering_rate;
...
float pid_calc_new(pid_t *pid, float get, float set)
{
  ...
  pid->pout = pid->p * pid->err[NOW];
  pid->iout += pid->i * pid->err[NOW];
  pid->dout = (1 - pid->filtering_rate)*(pid->err[NOW] - pid->err[LAST]) +  (pid->filtering_rate)*(pid->last_dout);
	pid->last_dout = pid->dout;
//	abs_limit((&pid->dout),300);
  abs_limit(&(pid->iout), pid->integral_limit);
  pid->out = pid->pout + pid->iout + pid->dout;
  abs_limit(&(pid->out), pid->max_output);
